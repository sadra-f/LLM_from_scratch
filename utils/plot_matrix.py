...
if __name__ == "__main__":
    ...
    n_layers = 4  # number of attention layers
    
    # Create sample attention weights (random matrices with softmax-like properties)
    attention_weights = []
    for layer in range(n_layers):
        # Create a random matrix with attention-like properties
        weights = np.random.rand(M, M)
        weights = weights / weights.sum(axis=1, keepdims=True)  # Row-normalize (like softmax)
        attention_weights.append(weights)
    
    # Example labels
    row_col_values = ['Token A', 'Token B', 'Token C', 'Token D', 'Token E']
    
    # Example 1: All layers in one figure with subplots
    print("Plotting all layers in one figure...")
    fig1 = plot_attention_heatmaps(
        attention_weights=attention_weights,
        row_col_values=row_col_values,
        title="Multi-Layer Attention Weights",
        figsize=(4, 4),
        cmap='Blues',
        vmin=0, vmax=0.5,
        layer_names=['Encoder Layer 1', 'Encoder Layer 2', 'Decoder Layer 1', 'Decoder Layer 2'],
        annot=True,
        fmt='.2f'
    )
    
    # Example 2: Separate windows for each layer
    print("\nPlotting each layer in separate windows...")
    figures = plot_attention_heatmaps(
        attention_weights=attention_weights,
        row_col_values=row_col_values,
    ...
    # Example 3: With custom color limits
    print("\nPlotting with custom color limits...")
    fig3 = plot_attention_heatmaps(
        attention_weights=attention_weights[:2],  # Just first 2 layers
        row_col_values=row_col_values,
        title="First Two Layers (Custom Limits)",
        cmap='RdYlBu',
        vmin=0.1,
        vmax=0.4,
        annot=True,
